...
func TestBinStreamWriter(t *testing.T) {
	p := WriterBinStream()
	a := byte(0xFF)
	b := uint16(0xFF00)
	c := uint32(0xFF0000)
	...
	tmp1, _ := reader.ReadUint16()
	if b != tmp1 {
		t.Error("packet readu16 mismatch")
	}

	tmp2, _ := reader.ReadUint24()
	if c != tmp2 {
		t.Error("packet readu24 mismatch")
	}

	tmp3, _ := reader.ReadUint32()
	if d != tmp3 {
		t.Error("packet readu32 mismatch")
	}

	tmp4, _ := reader.ReadUint64()
	if e != tmp4 {
...
func BenchmarkBinStreamWriter(b *testing.B) {
	for i := 0; i < b.N; i++ {
		p := WriterBinStream()
		p.WriteUint16(128)
		p.WriteBool(true)
